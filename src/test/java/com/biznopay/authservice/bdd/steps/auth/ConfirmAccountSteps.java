...
public class ConfirmAccountSteps {
    ...
    // SCENARIO: Successfully confirm account with valid accessToken
    @Given("a user registered with email {string} has a valid confirmation accessToken")
    ...
    @When("i send a confirmation request with the valid accessToken")
    public void iSendAConfirmationRequestWithTheValidToken() {
        ...
        scenarioContext.setResponse(
                scenarioContext.getRestTemplate().getForEntity(
                        url("/auth/confirm-account?accessToken=" + token.getId()),
                        ApiResponse.class
                )
        );
    }
    ...
    // SCENARIO: Reject confirmation with expired accessToken
    @Given("a user registered with email {string} has an expired confirmation accessToken")
    ...
    // SCENARIO: Reject confirmation with invalid or tampered accessToken
    ...
    @When("i send a confirmation request with the invalid accessToken")
    public void iSendAConfirmationRequestWithTheInvalidToken() {
        scenarioContext.setResponse(
                scenarioContext.getRestTemplate().getForEntity(
                        url("/auth/confirm-account?accessToken=invalidToken"),
                        ApiResponse.class
                )
        );
    }
    ...
    // SCENARIO: Reject confirmation when accessToken is missing
    @When("i send a confirmation request with the missing accessToken")
    public void iSendAConfirmationRequestWithTheMissingToken() {
        scenarioContext.setResponse(
                scenarioContext.getRestTemplate().getForEntity(
                        url("/auth/confirm-account?accessToken="),
                        ApiResponse.class
                )
        );
    }

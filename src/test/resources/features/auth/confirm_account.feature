Feature: Confirm Account
  ...
  * A missing token must be rejected
  ...
  Scenario: Successfully confirm account with valid token
    Given a user registered with email "user@example.com" has a valid confirmation accessToken
  ...
  Scenario: Reject confirmation with expired token
    Given a user registered with email "user@example.com" has an expired confirmation accessToken
    When i send a confirmation request with the valid accessToken
